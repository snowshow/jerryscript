                                                            ...
                                                            bool free_base_string) /**< free base string or not */
{
  ...
  if (start < end)
  {
    ...
    appended_string = ecma_concat_ecma_strings (base_string_p, substring_p);
  ...
  {
    appended_string = base_string_p;
  }
  else
  {
    appended_string = ecma_copy_or_ref_ecma_string (base_string_p);
  }
 ...
 */
static ecma_completion_value_t
ecma_builtin_string_prototype_object_replace_match (replace_search_context *context_p) /**< search context */
{
  ecma_completion_value_t ret_value = ecma_make_empty_completion_value ();
  ...
  if (context_p->is_regexp)
  {
    ...
    if (!ecma_is_value_null (match_value))
    {
      ...
      context_p->match_start = (ecma_length_t) (*ecma_get_number_from_value (index_value));
 ...
 */
static ecma_completion_value_t
ecma_builtin_string_prototype_object_replace_get_string (replace_search_context *context_p, /**< search context */
                                                         ecma_value_t match_value) /**< returned match value */
{
  ...
  if (context_p->is_replace_callable)
  {
    ...
    ecma_length_t length = (ecma_length_t) (*ecma_get_number_from_value (length_value));
 ...
 */
static ecma_completion_value_t
ecma_builtin_string_prototype_object_replace_loop (replace_search_context *context_p) /**< search context */
{
  ecma_completion_value_t ret_value = ecma_make_empty_completion_value ();
  ecma_length_t previous_start = 0;
...
/**
 * Generic helper function to analiyse the search value
 *
 * @return completion value
 *         Returned value must be freed with ecma_free_completion_value.
 */
static ecma_completion_value_t
ecma_builtin_string_prototype_object_replace_main (replace_search_context *context_p, /**< search context */
                                              ...
                                              ecma_value_t replace_value) /**< routine's second argument */
{
  ...
  replace_search_context context;
